...
// Fetch approved cars
$stmt = $pdo->prepare("SELECT * FROM cars WHERE status = 'approved'");
$stmt->execute();
$cars = $stmt->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Small helper to safely show an alert and redirect
    function alert_and_redirect($msg, $url = 'onmarkettest.php') {
        // json_encode ensures proper JS string escaping
        echo "<script>
    ...
    if (!$car_id || !$message) {
        alert_and_redirect('Please choose a vehicle and enter a message.');
    }

    try {
        // OPTION A: your table HAS a `type` column (seems intended)
        $stmt = $pdo->prepare("
            INSERT INTO inquiries (user_id, car_id, type, message, status)
            VALUES (:user_id, :car_id, :type, :message, :status)
        ");
        $stmt->execute([
            ':user_id' => $user_id,
            ':car_id' => $car_id,
        ...
        alert_and_redirect('Thanks! Your inquiry was sent successfully.');
    } catch (PDOException $e) {
        // In production you might log the detailed error and show a generic message.
        alert_and_redirect('Sorry, something went wrong: ' . $e->getMessage());
    }
}
...
<body>
  ...
  <main class="section-padding" style="display: flex; gap: 40px; align-items: flex-start; max-width: 1200px; margin: 100px auto 0; padding: 40px 20px;">
    ...
    <aside class="sticky-sidebar" style="flex: 1; background-color: #f8f9fa; padding: 30px; border-radius: 5px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);">
      ...
      <form action="onmarkettest.php" method="POST">
        <?php if ($isLoggedIn): ?>
          <input type="hidden" name="user_id" value="<?php echo $_SESSION['user_id']; ?>">
        <?php endif; ?>
        <div class="form-group">
          <label for="car_id" class="form-label">Vehicle of Interest</label>
