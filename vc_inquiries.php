<?php
// vc_inquiries.php — Top-of-file PHP (unified list + car-image override)
session_start();
...
/* --------------------------- Controls --------------------------- */
$q       = trim($_GET['q'] ?? '');
$bucket  = trim($_GET['bucket'] ?? '');   // '', 'car', 'gen', 'off'
...
$offmarket = safeFetch(
  $pdo,
  "SELECT id, vehicle, message, investment_type, status, created_at
   FROM offmarket_inquiries WHERE user_id = ? ORDER BY created_at DESC",
  [$uid]
);
...
// offmarket_inquiries (pool image)
foreach ($offmarket as $o) {
  $title = clean_title($o['vehicle'] ?: 'Off-Market Inquiry');
  $inv   = $o['investment_type'] ?? '';
  $sub   = $inv ? ('Investment: '.ucfirst($inv)) : 'Off-Market';
  ...
  $items[] = [
    ...
    'image'      => pick_image($imagePool, 'off-'.$o['id'].'-'.$inv),
    'open_url'   => 'view_offmarket_inquiry.php?id='.(int)$o['id'],
  ];
}
...
<body>
  ...
  <main>
    ...
    <section class="section-padding" style="margin-top:20px;">
      <div class="container">
        <form class="toolbar" method="get" action="vc_inquiries.php">
          ...
          <div class="field">
            ...
            <select name="bucket" aria-label="Filter by category">
              ...
              <option value="off" <?= $bucket==='off'?'selected':'' ?>>Off-Market</option>
            </select>
          </div>
          <div class="field">
            ✅
          ...
          <?php if ($q || $bucket || $statusF || $sort!=='newest'): ?>
            <a class="btn secondary" href="vc_inquiries.php">Reset</a>
          <?php endif; ?>
        </form>
        ...
        <?php else: ?>
          <div class="grid">
            <?php foreach ($items as $it):
              ...
              $bucketLabel = ['car'=>'Car Request','gen'=>'Inquiry','off'=>'Off-Market'][$it['bucket']] ?? 'Inquiry';
